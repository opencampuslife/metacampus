...
## RiskScorer GDExtension 技术要点（Phase 3, 2026-05-30）

### Godot C++ String 中文匹配不能用 find()/contains()
Godot 4 的 `String.find()` / `String.contains()` 按 Unicode codepoint 比较，与 UTF-8 字节序列不匹配。
- ASCII（如 "hello"）可以匹配
- 中文 UTF-8 字节序列永远不匹配（find() 返回 -1）
- **Solution**：JSON 解析后 keyword 变成 Godot `String`，但规则匹配用 `String.contains(keyword)` 是可靠的（Godot String 内部是 Unicode，contains 对 Unicode 支持正确）

### JSON 驱动的 RiskScorer 架构
```
data/rules/risk_rules.json (UTF-8)
  → FileAccess.open() + JSON.parse()
  → Godot Dictionary/Array
  → keyword 作为 Godot String 进入 C++
  → String.contains() 匹配（稳定）
```

### Phase 3.2 完成文件
- `scripts/gdscript/risk/risk_scorer_service.gd` — Autoload service
- `scripts/gdscript/ui/risk_review_panel.gd` — RiskReviewPanel script
- `scenes/risk/RiskReviewPanel.tscn`
- `native/risk_scorer/` — GDExtension C++ (SConstruct, gdextension, src/*.h/cpp)
...
```gdscript
{
  ...
  recommended_action: String ("allow"/"revise"/"escalate"/"block"),
  triggered_rules: PackedStringArray,
  compliance_delta: int,
  parent_trust_delta: int,
  stability_delta: int
}
```
